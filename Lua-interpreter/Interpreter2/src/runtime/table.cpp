...
{
    std::map<Config::T_OID, std::list<Pair> >::iterator iter = table.find(v->getValue());
    if (iter != table.end())
    {
        for (std::list<Pair>::iterator it = iter->second.begin(); it != iter->second.end(); ++it)
        {
			Pair &p = *it;
...
Table::eraseFromNumber(NumberValue *v)
{
    ...
    {
        for (
            std::list<Pair>::iterator it = iter->second.begin();
            it != iter->second.end();
            ++it
        ) {
            Pair &p = *it;
...
{
    std::map<Config::T_OID, std::list<Pair> >::iterator iter = table.find(v->getHash());
    if (iter != table.end())
    {
        for (
            std::list<Pair>::iterator it = iter->second.begin();
...
{
    std::map<Config::T_OID, std::list<Pair> >::iterator iter = table.find(v->getID());
    if (iter != table.end())
    {
        for (
            std::list<Pair>::iterator it = iter->second.begin();
